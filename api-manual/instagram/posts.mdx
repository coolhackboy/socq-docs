...
Provide either `usernames` or public Instagram profile `urls`. The legacy `query` field does not select posts and cannot be used on its own; use [Instagram Search](/api-manual/instagram/search) for keyword searches.

## Validation

A request containing only the legacy `query` field is rejected before task creation and is not billed:
...
Existing integrations may continue sending `query` together with valid `usernames` or `urls`; the request remains compatible, but `query` is ignored.
