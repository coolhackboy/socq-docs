...
```bash
socq task wait TASK_ID --timeout 180 --result-view standard
```
...
Successful tasks return records in `results.items`. V1 REST defaults to `full`, which preserves the existing normalized output. Request `standard` for the fixed resource-level schema shared by REST, MCP, CLI, SDK, and Skills.

Standard results retain declared nullable fields and exclude `raw` and unstable `extra`. Use `fields` only with `standard`; it accepts at most 50 comma-separated top-level fields or dot paths. A field is omitted only when the projection did not request it.

MCP and Skills should normally request `view: "standard"`; typed MCP tools use `_result_view` and `_result_fields`. CLI uses `--result-view standard` and optional `--result-fields`.
