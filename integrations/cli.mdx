...
socq task results TASK_ID --result-view standard --result-fields id,url,text,author.username --format csv --output results.csv
socq task files TASK_ID
```

`--result-view full` preserves the V1 compatible result. `--result-view standard` returns the fixed resource schema; pair it with optional `--result-fields` for a smaller projection.
...
Normal CLI submissions are recorded with `request_source="cli"`. When an Agent Skill falls back to the CLI, pass the source marker on the submission command:

```bash
socq youtube comments --urls "https://www.youtube.com/watch?v=VIDEO_ID" --request-source skill
```

`SOCQ_REQUEST_SOURCE=skill` provides the same behavior. New tasks are then recorded as `skill-cli`; task polling commands do not need the marker.
