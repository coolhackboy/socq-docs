...
socq task results TASK_ID --result-fields id,url,text,author.username --format csv --output results.csv
socq task files TASK_ID
```

Task results use the fixed resource schema. Use optional `--result-fields` for a smaller projection.
