...
成功任务在 `results.items` 中返回记录。V1 REST 默认使用 `full`，保留现有标准化输出；请求 `standard` 可获得 REST、MCP、CLI、SDK 与 Skills 共用的资源级固定 Schema。

标准结果会保留已声明的可空字段，并排除 `raw` 和不稳定的 `extra`。`fields` 只能与 `standard` 一起使用，最多接受 50 个逗号分隔的顶层字段或点路径。只有调用方未通过投影请求字段时，该字段才会省略。

MCP 与 Skills 通常应请求 `view: "standard"`；Typed MCP 工具使用 `_result_view` 和 `_result_fields`，CLI 使用 `--result-view standard` 和可选的 `--result-fields`。
