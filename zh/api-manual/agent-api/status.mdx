...
| `results.schema_id` | 字符串 | 资源 Schema ID |
| `results.schema_version` | 字符串 | 资源 Schema 版本 |
...
| `fields` | 字符串 | `null` | 最多 50 个逗号分隔字段或点路径。 |

除非被 `fields` 投影省略，否则所有声明字段都会保留。不可获得的标量和对象返回 `null`；确定为零、假值和空集合时分别保留 `0`、`false` 和 `[]`。
